    ...
    const onPointerUp = (ev: PointerEvent) => {
      ...
      if (funnelStep === 0) {
        if (currentTranslateX >= maxMove * 0.5) {
          ...
          knob.style.transition = 'transform 0.3s cubic-bezier(0.16, 1, 0.3, 1)';
    ...
    <div className="app-shell">
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent !important; user-select: none !important; }
        ...
        .app-shell { width: 100%; max-width: 1280px; margin: 0 auto; background: var(--apple-titanium); min-height: 100vh; position: relative; z-index: 1; display: flex; flex-direction: column; }
        ...
        .phone-badge { display: inline-flex; align-items: center; gap: 7px; background: var(--card-pure-white); border: 1px solid var(--apple-border); padding: 8px 16px; border-radius: 999px; color: var(--apple-blue); font-size: 0.82rem; font-weight: 600; text-decoration: none; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.03); }
        ...
        .h-pill { display: inline-flex; align-items: center; gap: 8px; padding: 9px 18px; background: var(--card-pure-white); border: 1px solid var(--apple-border); border-radius: 999px; font-size: 0.76rem; font-weight: 700; color: var(--apple-dark); box-shadow: 0 4px 14px rgba(0, 0, 0, 0.03); }
        .h-pill span { color: var(--apple-blue); }
        .hero-cta-btn { display: inline-flex; align-items: center; justify-content: center; background: var(--apple-blue); color: #ffffff; padding: 16px 32px; border-radius: 999px; font-size: 0.94rem; font-weight: 700; text-decoration: none; box-shadow: 0 8px 24px rgba(0, 113, 227, 0.35); width: fit-content; }
        .hero-box { position: relative; width: 100%; max-width: 380px; margin: 0 auto; border-radius: 24px; overflow: hidden; aspect-ratio: 9 / 16; background: #0d131f; box-shadow: 0 24px 60px -10px rgba(0, 0, 0, 0.25); border: 1px solid var(--apple-border); }
        ...
        .swipe-interactive-zone { position: absolute; bottom: 24px; left: 20px; right: 20px; height: 48px; background: rgba(0, 0, 0, 0.25) !important; border: 1px solid rgba(255, 255, 255, 0.2) !important; backdrop-filter: blur(12px); border-radius: 999px; display: flex; align-items: center; padding: 0 4px; z-index: 20; touch-action: none; transition: opacity 0.3s ease; }
        .swipe-interactive-zone::after { content: "SWIPE FOR SMILE REVEAL ➔"; position: absolute; width: 100%; text-align: center; font-size: 0.65rem; font-weight: 800; letter-spacing: 0.14em; color: rgba(255, 255, 255, 0.7); pointer-events: none; }
        .swipe-interactive-zone.reverse::after { content: "⬅ PULL TO RESET PREVIEW"; }
        .swipe-arrow-handle { height: 40px; width: 52px; background: rgba(255, 255, 255, 0.92); border: 1px solid #ffffff; border-radius: 999px; display: flex; align-items: center; justify-content: center; cursor: grab; position: absolute; left: 4px; top: 50%; transform: translate3d(0, -50%, 0); z-index: 25; box-shadow: 0 4px 14px rgba(0, 0, 0, 0.25); }
        .swipe-arrow-handle svg { width: 18px; height: 18px; fill: var(--apple-blue); transition: transform 0.3s ease; pointer-events: none; }
        .half-form-drawer { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(255, 255, 255, 0.98); backdrop-filter: blur(24px); border-radius: 20px 20px 0 0; box-shadow: 0 -12px 40px rgba(0, 0, 0, 0.18); z-index: 40; display: flex; flex-direction: column; padding: 24px 20px; transform: translate3d(0, 100%, 0); transition: transform 0.38s cubic-bezier(0.16, 1, 0.3, 1); border-top: 1px solid var(--apple-border); }
        ...
        .drawer-input { width: 100%; background: var(--apple-titanium); border: 1px solid var(--apple-border); padding: 12px 14px; border-radius: 12px; font-size: 0.88rem; color: var(--apple-dark); margin-bottom: 10px; outline: none; font-family: inherit; }
        .drawer-btn { width: 100%; background: var(--apple-blue); border: none; padding: 13px; border-radius: 12px; color: #ffffff; font-size: 0.92rem; font-weight: 600; cursor: pointer; font-family: inherit; }
        ...
        .animate-on-scroll { opacity: 0; transform: translateY(24px); transition: opacity 0.7s cubic-bezier(0.16, 1, 0.3, 1), transform 0.7s cubic-bezier(0.16, 1, 0.3, 1); will-change: opacity, transform; }
        ...
        .doctor-card { position: relative; border-radius: 18px; overflow: hidden; margin-bottom: 22px; background: var(--card-pure-white); border: 1px solid var(--apple-border); box-shadow: 0 10px 30px rgba(15, 23, 42, 0.04); }
        .doc-img { width: 100%; height: 380px; object-fit: cover; object-position: top center; display: block; }
        .doc-tag { position: absolute; top: 16px; right: 16px; background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(12px); padding: 7px 16px; border-radius: 999px; font-size: 0.74rem; font-weight: 700; color: var(--apple-blue); }
        .stats-row { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-top: 22px; }
        .stat-pill { background: var(--card-pure-white); border: 1px solid var(--apple-border); border-radius: 16px; padding: 18px 14px; text-align: center; box-shadow: 0 6px 20px rgba(15, 23, 42, 0.03); }
        ...
        .acc-badge { background: var(--card-pure-white); border: 1px solid var(--apple-border); color: var(--apple-dark); font-size: 0.76rem; font-weight: 600; padding: 8px 18px; border-radius: 999px; display: inline-flex; align-items: center; gap: 6px; }
        .infinite-marquee-box { width: 100%; overflow: hidden; position: relative; margin: 8px 0 16px 0; cursor: grab; touch-action: pan-x; mask-image: linear-gradient(to right, transparent, black 6%, black 94%, transparent); -webkit-mask-image: linear-gradient(to right, transparent, black 6%, black 94%, transparent); }
        .infinite-marquee-inner { display: flex; gap: 18px; width: max-content; will-change: transform; }
        .review-bubble { width: 290px; flex-shrink: 0; background: var(--card-pure-white); border: 1px solid var(--apple-border); border-radius: 18px; padding: 20px; box-shadow: 0 8px 24px rgba(15, 23, 42, 0.04); }
        ...
        .interactive-cases-box { width: 100%; overflow: hidden; position: relative; margin: 8px 0 16px 0; cursor: grab; touch-action: pan-x; mask-image: linear-gradient(to right, transparent, black 6%, black 94%, transparent); -webkit-mask-image: linear-gradient(to right, transparent, black 6%, black 94%, transparent); }
        .interactive-cases-inner { display: flex; gap: 18px; width: max-content; will-change: transform; }
        .case-card-stream { width: 260px; flex-shrink: 0; background: var(--card-pure-white); border: 1px solid var(--apple-border); border-radius: 18px; padding: 14px; box-shadow: 0 8px 24px rgba(15, 23, 42, 0.04); }
        .case-photo-slot { position: relative; border-radius: 12px; overflow: hidden; height: 190px; margin-bottom: 10px; background: #e2e5e9; }
        .case-photo-slot img { width: 100%; height: 100%; object-fit: cover; display: block; }
        .faq-list { display: flex; flex-direction: column; gap: 12px; }
        .faq-item { background: var(--card-pure-white); border: 1px solid var(--apple-border); border-radius: 16px; overflow: hidden; box-shadow: 0 6px 20px rgba(15, 23, 42, 0.03); }
        ...
        .consult-card { background: var(--card-pure-white); border: 1px solid var(--apple-border); border-radius: 20px; padding: 28px 24px; box-shadow: 0 12px 32px rgba(15, 23, 42, 0.05); margin-bottom: 26px; }
        .slots-pill { display: inline-flex; align-items: center; gap: 7px; background: #fff8eb; border: 1px solid #ffe2b3; padding: 6px 14px; border-radius: 999px; font-size: 0.75rem; font-weight: 600; color: #b25e00; margin-bottom: 16px; }
        .slots-dot { width: 6px; height: 6px; border-radius: 50%; background: #ff9500; }
        .guarantee-strip { display: flex; flex-direction: column; gap: 7px; background: #f2faf4; border: 1px solid #d1edd8; border-radius: 14px; padding: 14px 16px; margin-bottom: 18px; font-size: 0.78rem; font-weight: 600; color: #248a3d; }
        .form-input { width: 100%; background: var(--apple-titanium); border: 1px solid var(--apple-border); padding: 14px 16px; border-radius: 14px; font-size: 0.92rem; color: var(--apple-dark); margin-bottom: 12px; outline: none; font-family: inherit; }
        .btn-confirm { width: 100%; background: var(--apple-blue); border: none; padding: 15px; border-radius: 14px; color: #ffffff; font-size: 0.96rem; font-weight: 600; cursor: pointer; font-family: inherit; box-shadow: 0 4px 16px rgba(0, 113, 227, 0.3); }
        
        /* Interactive Map Preview Card Style Matching Screenshot 26_3 */
        .map-preview-card { position: relative; border-radius: 24px; overflow: hidden; border: 1px solid var(--apple-border); margin-bottom: 40px; box-shadow: 0 12px 36px rgba(15, 23, 42, 0.08); background: #ffffff; display: flex; flex-direction: column; }
        ...
        .map-floating-badge { position: absolute; top: 16px; left: 16px; background: #ffffff; border-radius: 12px; padding: 12px 16px; box-shadow: 0 6px 20px rgba(0, 0, 0, 0.15); display: flex; align-items: center; justify-content: space-between; gap: 16px; max-width: 320px; width: calc(100% - 32px); z-index: 10; border: 1px solid rgba(0,0,0,0.06); }
        ...
        .map-icon-btn { width: 32px; height: 32px; border-radius: 8px; background: #f0f4fd; border: 1px solid rgba(0, 113, 227, 0.2); display: flex; align-items: center; justify-content: center; color: var(--apple-blue); text-decoration: none; font-size: 0.9rem; transition: background 0.2s; }
        .map-icon-btn:hover { background: #e2ecfc; }

        .floating-ai { position: fixed !important; bottom: 20px !important; right: 20px !important; z-index: 999999 !important; background: rgba(255, 255, 255, 0.95); border: 1.5px solid rgba(0, 113, 227, 0.35); width: 48px; height: 48px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 22px rgba(0, 0, 0, 0.16); backdrop-filter: blur(14px); cursor: pointer; }
        ...
        .ai-msg { max-width: 84%; padding: 11px 16px; border-radius: 16px; font-size: 0.85rem; line-height: 1.45; }
        ...
        .ai-chat-input { flex: 1; border: 1px solid var(--apple-border); background: var(--apple-titanium); border-radius: 999px; padding: 10px 16px; font-size: 0.86rem; outline: none; font-family: inherit; }
      ...
      <div className="content-container">
        ...
        <section className="section-padding animate-on-scroll" id="consultation-area" style={{ paddingTop: 0 }}>
          ...
          {/* Interactive Google Maps Preview Card matching Screenshot 26_3 */}
